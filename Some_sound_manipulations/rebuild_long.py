...
for c in carriers:
    ...
    while location<len(c):
        ...
        this_carrier=np.concatenate([this_carrier,maybe_carrier,maybe_carrier])
    long_carriers.append(this_carrier)

s_rebuild=zeros(2*(len(s0)))

print "rebuilding",
for i,envelope in enumerate(long_envelopes):
    print i,
    while len(long_carriers[i])<len(envelope):
        long_carriers[i]=np.append(long_carriers[i],np.asarray([0.0]))
    for t,e in enumerate(envelope):
        s_rebuild[t]+=long_carriers[i][t]*e
print "\n"
...
wavfile.write('rebuild_long_'+filename,samp_freq,rebuild_snd)
